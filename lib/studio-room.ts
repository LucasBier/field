...
export function createStudioRoom(
  ...
  box(1.22, 0.24, 2.9, green, [-3.75, 0.33, 0.3], 0.1).userData.zuriActivity =
    'read';
  ...
  for (let i = 0; i < 3; i++) {
    box(
      ...
      0.08,
    ).userData.zuriActivity = i === 2 ? 'rest' : 'read';
    const pillow = box(
      0.18,
      0.49,
      0.63,
  ...
  box(0.79, 0.48, 0.035, black, [-2.82, 1.19, -3.42]).userData.zuriActivity =
    'code';
  ...
  box(
    ...
    screenMaterial,
    [-2.82, 1.19, -3.398],
  ).userData.zuriActivity = 'code';
  const cursor = box(0.008, 0.026, 0.004, paper, [-2.61, 1.1, -3.39]);
  cursor.visible = false;
  let zuriActivity: RoomActivity | null = null;
  let activityTime = 0;
  ...
  pen.rotation.z = -0.3;
  box(0.6, 0.12, 0.6, violet, [-2.8, 0.46, -2.3], 0.06).userData.zuriActivity =
    'code';
  box(0.59, 0.5, 0.1, violet, [-2.8, 0.74, -2.04], 0.06).userData.zuriActivity =
    'code';
  ...
  return {
    ...
    update(dt: number, camera: THREE.Camera, reduced: boolean) {
      ...
      screenMaterial.emissiveIntensity = zuriActivity === 'code' ? 0.7 : 0.35;
