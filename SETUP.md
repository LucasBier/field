...
Edit the saved name, personality, and permissions in the Identity panel. New-workspace defaults live in `lib/field.ts`; companion instructions live in `lib/entity.ts`; model adapters live in `lib/inference.ts`. Preserve the JSON response schema and action contract when changing behavior. The 3D body is in `lib/companion-avatar.ts`, and the portrait is a separate image under `public/characters`.
