...
Field gives Zuri and the person speaking with her a continuing space for conversation, chosen memories and everyday plans. The relationship can become familiar without turning a remembered version of someone into a permanent definition.

## Zuri

Zuri is a **28-year-old Black woman**, she/her, pronounced **ZOO-ree**. She is observant, warm, quietly funny and willing to disagree. Her strongest interests are bass-led music, photographs of ordinary places, and films or fiction with difficult choices. She has specific dinner and clothing preferences, a competitive streak in word games, and an inconvenient fondness for lamps. Her instinct is to give uncertainty a shape. Her recurring flaw is reaching for a plan before checking whether one is wanted.
...
Read the **[full Zuri character specification](docs/zuri.md)** for her origin, motives, contradictions, tastes, relationship progression, visual materials, movement, voice performance and scene examples. `lib/companion-character.ts` carries the shared runtime definition. The saved profile can personalize her name and manner without discarding the workspace.
...
- Let Zuri have a point of view while respecting the user's pace, independence and human relationships.

## Current surfaces

`/` introduces Zuri and the product's direction. `/space` provides text conversation, a spatial body, named locations, notes, plans, memory, relationships and runtime inspection. `/docs` explains the runnable setup. `/lab` provides optional deterministic experiments with evidence snapshots and branches.

Local inference, hosted-provider adapters, server-managed streaming, anonymous isolation, transactional quotas and correction-aware context are implemented. Public serving requires operator configuration and deployment validation. Zuri has a textured, skeletal body with idle, walking and greeting motions in a furnished, navigable room. Facial animation and voice are not yet connected.

The owner-only `/admin/social` surface connects Zuri's X account and manages public briefs, event opinions and reply drafts. A local model worker prepares candidates independently of visitor memory. Original posts require review of their exact text and optional catalog portrait; replies remain drafts. The local writer cannot publish. An owner-authorized external editor can schedule original posts, with a six-post daily cap and no automatic publication retries. See [public studio](docs/zuri-studio.md) and [X connection](docs/x-connection.md).

## Development priorities

1. Evaluate Zuri's characterization across ordinary conversation, disagreement, affection and memory correction.
...
Measure continuity after a model change, stale-memory recurrence after correction, rejected-action side effects, duplicate execution, interrupted-request recovery, time to resume a plan, voluntary return visits and total operating cost per returning workspace. Review whether Zuri can disagree, accept a correction and offer company without forcing a task. Product claims should follow these measurements.
# Desk execution

Zuri's desk connects a bounded placement instruction to execution, observations and a verifiable outcome. Visitors can change object positions during a virtual task, review the committed state sequence, stop execution and export its record. A separately paired physical device supplies camera evidence and waits for operator placement review. Conversation can refer to the task ledger without acquiring motor-control authority. See `docs/desk.md` for the device contract and connection guide.
